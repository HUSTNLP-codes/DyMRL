...
class Basemodel(KGModel):
    def __init__(self, args):
        super(Basemodel, self).__init__(args.sizes, args.rank, args.dropout, args.gamma, args.bias, args.init_size)
...
class AdaptiveCurvatureDistance(Basemodel):
    def __init__(self, args):
        super(AdaptiveCurvatureDistance, self).__init__(args)
...
class DyMRL(AdaptiveCurvatureDistance):
    def __init__(self, args):
        ...
        linguistic = f"../pretrain/{args.dataset}/matrix_linguistic_{args.txtenc}.npy"
        visual = f"../pretrain/{args.dataset}/matrix_visual_{args.figenc}.npy"
        tm_ling, tm_visual = torch.tensor(np.load(linguistic)), torch.tensor(np.load(visual))
        self.img_vec = tm_visual.to(torch.float32).transpose(0, 1).cuda()
        self.img_dimension = self.img_vec.shape[-1]
        self.ling_vec = tm_ling.to(torch.float32).cuda()
        self.ling_dimension = self.ling_vec.shape[-1]
        ...
        self.RNN = nn.GRUCell(self.rank, self.rank)
        ...
        self.word_emb_h = nn.Embedding((self.sizes[2]-self.sizes[0]), self.rank)
        self.word_emb_h.weight.data = self.init_size * torch.randn(((self.sizes[2]-self.sizes[0]), self.rank))
    ...
    def build_layers(self, args):
        for i in range(self.n_layers):
            ...
            self.layers_c.append(CGCNLayer(self.rank, self.sizes[0], self.sizes[1], self.rel, self.en_dropout, args.en_loop, self.init_size, args.en_bias))
            self.layers_h.append(HGCNLayer(self.rank, self.sizes[1], self.en_dropout, self.get_queries, args.en_loop, self.init_size, args.en_bias))
    ...
    def evolve(self, g_list, static_graph):
        ...
        word_emb = torch.cat((self.h, self.word_emb_h.weight), dim=0)
        ...
        for idx in range(len(g_list)):
            if g_list[idx].device == "cpu":
                ...
                g = g_list[idx]
            hidden_e = self.snap_forward_e(g, self.static_h)
            hidden_h = self.snap_forward_h(g, self.h)
            hidden_c = self.snap_forward_c(g, self.h)
            # print(hidden_e.shape, hidden_h.shape, hidden_c.shape)
            structural_emb = self.update([hidden_e, hidden_h, hidden_c])[-1]
            structural_emb = self.RNN(structural_emb, self.init_ent_emb.weight)
            visual_emb = torch.mm(self.img_vec, self.mats_img)
            visual_emb = self.RNN(visual_emb, self.init_ent_emb.weight)
            linguistic_emb = torch.mm(self.ling_vec, self.mats_ling)
            linguistic_emb = self.RNN(linguistic_emb, self.init_ent_emb.weight)

            evolve_emb = self.update([structural_emb, visual_emb, linguistic_emb])[-1]
            evolve_embs.append(evolve_emb)
        output_emb = self.update(evolve_embs)[-1]
        return output_emb
    ...
    def snap_forward_c(self, g, in_ent_emb):
        node_id = g.ndata['id'].squeeze()
        g.ndata['h'] = in_ent_emb[node_id]
